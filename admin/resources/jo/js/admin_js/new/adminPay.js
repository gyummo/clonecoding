...
/*----------------------*/
var openUserInfoform = function (idx) {
    $.ajax({
        type: 'POST',
        data: { idx: idx },
        url: ('/admin/shopping/mileage/user_info.cm'),
        dataType: 'html',
        async: false,
        cache: false,
        success: function (html) {
            $.cocoaDialog.open({ type: 'admin_user_info', custom_popup: html });
        }
    });
};
SHOP_MEMBER_POINT.init('/admin/shopping/mileage/?q=YjowOw%3D%3D', '/admin/shopping/mileage');
/*----------------------*/
/*----------------------*/
