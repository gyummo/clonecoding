...
$(function () {
    ...
    const handler = datepickerButtonHandler({
        startDate: isValidDate(__DATA__.startDate) ? __DATA__.startDate : __DATA__.today,
        endDate: isValidDate(__DATA__.endDate) ? __DATA__.endDate : __DATA__.today,
    });

    handler.init();

    handler.onClick(function (self, startDate, endDate) {
    ...
    const fetchSiteStatList = (type) => {
        const { start, end } = handler.hyphenText();

        siteStat.getSiteStatList($("#site_stat_list"), type, start, end);
    }

    const fetchSiteStatUrlList = (type) => {
        const { start, end } = handler.hyphenText();

        siteStatUrl.getSiteStatList($("#site_stat_url_list"), type, start, end);
    }

    fetchSiteStatList(__DATA__.type);
    fetchSiteStatUrlList(__DATA__.type);

    getScrollPositionBottom('', '', function (is_bottom) {
        if (is_bottom) {
            fetchSiteStatList(__DATA__.type);
            fetchSiteStatUrlList(__DATA__.type);
        }
...
/*----------------------*/
/*----------------------*/
var nudge_point_modal = new NudgePointModal({
    image: "/admin/img/nudge_point_ac_image_v3.png",
    moImage: "/admin/img/nudge_point_ac_image_mo_v3.png",
    background: "/admin/img/nudge_point_ac_bg.png",
    title: "실시간 로그 분석으로<br/>매출을 높여보세요",
    description: "에이스카운터로 유입부터 이탈까지 분석하고 전환율을 높여보세요",
    url: "/admin/stat/additional_service?initial_product=acecounter",
    key: "site"
});

nudge_point_modal.open()
/*----------------------*/
/*----------------------*/
