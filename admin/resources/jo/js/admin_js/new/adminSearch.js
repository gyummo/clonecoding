...
$(function () {
    ...
    const fetchKeywordStatList = (type) => {
        const { start, end } = handler.hyphenText();

        keywordStat.getKeywordStatList($("#site_keyword_list"), type, start, end);
    }

    fetchKeywordStatList(__DATA__.type);

    if ($('#site_keyword_list').outerHeight() < $(window).outerHeight()) {
        fetchKeywordStatList(__DATA__.type);
    }
